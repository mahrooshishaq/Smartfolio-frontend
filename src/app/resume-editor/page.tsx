      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="grid items-start gap-6 xl:grid-cols-12">
          <main className="space-y-5 xl:col-span-8">
            ...
            <EditorSection title="Experience" onAdd={() => addCollectionItem('experience')} emptyText="No experience roles yet. Add a role to start building this section.">
          ...
          <aside className="sticky top-6 space-y-5 xl:col-span-4">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                {!applicableImprovements.length && <p className="rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">No safe one-click edits are available. Use the guidance in each section to edit manually.</p>}
              </div>
            </section>
          </aside>
        </div>
...
function EditorSection({ title, children, onAdd, emptyText }: { title: string; children: React.ReactNode; onAdd?: () => void; emptyText?: string }) { const hasContent = React.Children.count(children) > 0 && !(Array.isArray(children) && children.length === 0); return <section className="rounded-3xl bg-white p-5 shadow-sm md:p-7"><div className="mb-5 flex items-center justify-between"><h2 className="font-century text-lg font-black text-slate-800">{title}</h2>{onAdd && <button type="button" onClick={onAdd} className="flex items-center gap-1 rounded-lg bg-indigo-50 px-3 py-2 text-[10px] font-bold text-indigo-600"><Plus size={13} /> Add</button>}</div>{hasContent ? <div className="space-y-4">{children}</div> : <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-5 text-sm font-semibold text-slate-500">{emptyText || 'Nothing here yet.'}</p>}</section>; }
function EntryCard({ children, onRemove }: { children: React.ReactNode; onRemove: () => void }) { return <div className="relative rounded-2xl border border-slate-100 p-4"><button type="button" onClick={onRemove} className="absolute right-3 top-3 rounded-lg p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-500"><Trash2 size={14} /></button><div className="space-y-3 pr-8">{children}</div></div>; }
function EditableField({ label, value, onChange, multiline = false, tone, onFocus }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean; tone: string; onFocus: () => void }) { const common = `w-full rounded-xl border px-3 py-2.5 text-sm leading-6 text-slate-700 outline-none transition focus:border-indigo-400 focus:bg-white ${tone}`; const rows = Math.min(12, Math.max(4, Math.ceil(value.length / 78) + (value.match(/\n/g)?.length || 0))); return <label className="block w-full min-w-0"><span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-slate-500">{label}</span>{multiline ? <textarea value={value} rows={rows} onFocus={onFocus} onChange={event => onChange(event.target.value)} className={`${common} min-h-[7rem] resize-y overflow-auto`} /> : <input value={value} onFocus={onFocus} onChange={event => onChange(event.target.value)} className={common} />}</label>; }
function TagEditor({ label, values, placeholder, onChange }: { label: string; values: string[]; placeholder: string; onChange: (values: string[]) => void }) { const [draft, setDraft] = useState(''); const add = (raw: string) => { const incoming = parseSuggestedTags(raw); if (!incoming.length) return; onChange(mergeTags(values, incoming)); setDraft(''); }; return <div><span className="mb-2 block text-[9px] font-black uppercase tracking-wider text-slate-500">{label}</span><div className="flex min-h-14 flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 focus-within:border-indigo-300 focus-within:bg-white">{values.map(value => <button key={value.toLowerCase()} type="button" onClick={() => onChange(values.filter(item => item !== value))} title={`Remove ${value}`} className="group inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-red-200 hover:bg-red-50">{value}<X size={12} className="text-slate-500 group-hover:text-red-500" /></button>)}<input value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ',') { event.preventDefault(); add(draft); } else if (event.key === 'Backspace' && !draft && values.length) onChange(values.slice(0, -1)); }} onBlur={() => draft.trim() && add(draft)} placeholder={values.length ? 'Add another…' : placeholder} className="min-w-40 flex-1 bg-transparent px-2 py-1.5 text-sm text-slate-700 outline-none placeholder:text-slate-400" /></div><p className="mt-2 text-[10px] text-slate-500">Press Enter or comma to add. Click a bubble to remove it.</p></div>; }
function BulletEditor({ label, values, onChange, fieldProps }: { label: string; values: string[]; onChange: (values: string[]) => void; fieldProps: (value: string) => { tone: string; onFocus: () => void } }) { return <div><div className="mb-3 flex items-center justify-between"><span className="text-[9px] font-black uppercase tracking-wider text-slate-500">{label}</span><button type="button" onClick={() => onChange([...values, ''])} className="rounded-lg bg-indigo-50 px-2.5 py-1.5 text-[10px] font-bold text-indigo-600">+ Add bullet</button></div><div className="space-y-3">{values.map((value, index) => <div key={index} className="flex w-full items-start gap-2 rounded-2xl bg-slate-50/70 p-2"><div className="min-w-0 flex-1"><EditableField label={`Bullet ${index + 1}`} value={value} multiline onChange={next => onChange(values.map((item, itemIndex) => itemIndex === index ? next : item))} {...fieldProps(value)} /></div><button type="button" onClick={() => onChange(values.filter((_, itemIndex) => itemIndex !== index))} className="mt-6 shrink-0 rounded-lg p-2 text-slate-500 hover:bg-red-50 hover:text-red-500" aria-label={`Remove bullet ${index + 1}`}><Trash2 size={15} /></button></div>)}</div></div>; }
